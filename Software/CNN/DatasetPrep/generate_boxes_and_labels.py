"""
Interactive: asks how many author folders to process, then for each page
in those folders generates:
...
folder so you can inspect results before deciding to run this for real
across the dataset.

Standalone: no local imports, only numpy/PIL/pytesseract (need installing,
same as other scripts here).

Usage:
    python generate_boxes_and_labels.py
        (prompts: "How many author folders? ")
    python generate_boxes_and_labels.py --num-folders 10
    python generate_boxes_and_labels.py --num-folders 10 --out my_review_folder
    python generate_boxes_and_labels.py --num-folders 10 --include-150
"""
...
# =====================================================================
# Driver: ask how many author folders, process that many, save outputs
# =====================================================================
SCRIPT_DIR = Path(__file__).resolve().parent
# NOTE: this file lives in DatasetPrep/, one level deeper than Software/CNN/
# itself, so it takes an extra .parent hop to reach the repo root / the
# shared NOGIT/ folder (which stays at Software/CNN/NOGIT, not a new
# DatasetPrep/NOGIT).
...
def main():
    parser = argparse.ArgumentParser(description="Generate bounding-box preview images + labels.txt for N author folders.")
    ...
    parser.add_argument("--last-page-only", action="store_true",
                         help="Only process the LAST page (sorted) of each author folder, instead of every page.")
    args = parser.parse_args()

    num_folders = args.num_folders
    if num_folders is None:
        while True:
            raw = input("How many author folders would you like to process? ").strip()
    ...
    print(f"[Generate] Processing {len(selected)} author folder(s) "
          f"(out of {len(author_folders)} available): {selected}")
    ...
    for author_id in selected:
        ...
        if args.last_page_only and page_paths:
